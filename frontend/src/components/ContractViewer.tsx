...
export const ContractViewer: React.FC<ContractViewerProps> = ({ contractId, versionId, onBack }) => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)] bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden">
      {/* Header Controls Bar */}
      <div className="flex flex-wrap items-center justify-between p-3 border-b border-slate-800 bg-slate-900/90 gap-4 z-30">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-slate-400">Clause Tree • Risk Assessment • Grounded Q&A • Citations</div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 bg-slate-800/80 px-3 py-1.5 rounded-xl border border-slate-700">
      ...
      {warningMessage && (
        <div className="bg-amber-500/20 text-amber-300 text-xs px-4 py-2 border-b border-amber-500/30 flex items-center gap-2">
      ...
      {errorMessage && (
        <div className="p-4 text-center bg-red-500/10 border-b border-red-500/30 text-red-300 text-xs flex items-center justify-center gap-2">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        {/* Center: PDF Viewer */}
        <div className="flex-1 overflow-auto p-6 bg-slate-950 flex justify-center items-start relative">
          <div
            ref={containerRef}
            className="relative bg-white rounded-lg shadow-2xl overflow-hidden border border-slate-800"
        ...
        <div className="w-[450px] border-l border-slate-800 bg-slate-900 flex flex-col overflow-hidden">
          {activeRightTab === 'risk' && (
            <RiskAnalysisPanel
          ...
          {activeRightTab === 'inspector' && !selectedClause && (
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-slate-500 text-xs">
              <FileSearch className="w-10 h-10 mb-2 opacity-30 text-indigo-400" />
              <p className="font-semibold text-slate-400">No Clause Selected</p>
              <p className="mt-1 max-w-xs">Select a clause from the left document outline to view details, cross-references, and defined terms.</p>
            </div>
          )}
        </div>
      </div>
